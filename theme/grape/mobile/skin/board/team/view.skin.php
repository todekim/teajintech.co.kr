...
.subimg p {font-size: 16px;font-weight: 500;color: #2a2a2a;}

div.link_yt {text-align:center;}
div.link_yt a.demo{
    display:inline-block;
    background:none;
    color:#4ad1d4 !important;
    border:2px solid #4ad1d4;
    padding:0 25px;
    line-height:45px;
    border-radius:30px;
...
<section id="bo_v_atc">
    ...
    if($v_img_count) {
        ...
        if($bo_table == 'b_15' && $wr_id == '1') {
            ?>
            <a href="http://taejintech.co.kr/bbs/board.php?bo_table=b_17&wr_id=23" target="_blank">
                <div class="subimg">
                    <img src="/images/MC-1015Sv2.jpg">
                    <p>온도경보기 통신BOX(MC-1015S)</p>
                </div>
            </a>
            <a href="http://taejintech.co.kr/bbs/board.php?bo_table=b_17&wr_id=22" target="_blank">
                <div class="subimg">
                    <img src="/images/MC-1012NSv2.jpg">
                    <p>온도경보기 통신 BOX(MC-1012NS)</p>
                </div>
            </a>
            <?
        }
    ...
    <div id="bo_v_con">
        ...
        <?php
        for($i = 1; $i < 6; $i++ ){
            if($view['wr_link_yt_'.$i]){
                $link_yt = '';

                $url = $view['wr_link_yt_'.$i];

                if(!preg_match("/com/", $url)){

                    $link_yt = end(explode("/", $url));

                } else {

                    $parsed_url = parse_url($view['wr_link_yt_'.$i]);
                    parse_str($parsed_url['query'], $params);
                    $link_yt = $params['v'];

                }
        ?>
            <a id="show_modal" class="demo" href="https://youtube.com/embed/<?php echo $link_yt;?>" data-fancybox="youtube" data-width="1000"><?php echo $view['wr_title_yt_'.$i];?></a>
            <?php
            }
        }
        ?>
